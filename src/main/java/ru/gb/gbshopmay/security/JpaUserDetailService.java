...
import ru.gb.gbshopmay.dao.security.VerificationTokenDao;
import ru.gb.gbshopmay.entity.VerificationToken;
import ru.gb.gbshopmay.entity.enums.AccountStatus;
import ru.gb.gbshopmay.entity.security.AccountRole;
import ru.gb.gbshopmay.entity.security.AccountUser;
import ru.gb.gbshopmay.exception.UsernameAlreadyExistsException;
...
public class JpaUserDetailService implements UserDetailsService, UserService {
    ...
    public void createVerificationToken(UserDto user, String token) {
        ...
        VerificationToken myToken = VerificationToken.builder().token(token).user(userMapper.toAccountUser(user)).expiryDate(new Date(cal.getTime().getTime())).build();
        verificationTokenDao.save(myToken);
    }
    @Override
    public VerificationToken getVerificationToken(String token) {
        return verificationTokenDao.findVerificationTokenByToken(token);
    }
